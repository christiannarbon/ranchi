...
def join_group(group_id: int, request: JoinGroupRequest, db: Session = Depends(get_db)):
    ...
    if user.daily_status != "Looking":
        raise HTTPException(status_code=400, detail="User status must be 'Looking' to join a group")
    ...
    if current_members >= 6:
        raise HTTPException(status_code=400, detail="Group already has the maximum of 6 members")
