...
def nominate(group_id: int, request: NominateRequest, db: Session = Depends(get_db)):
    ...
    if group.is_locked:
        raise HTTPException(status_code=400, detail="Cannot nominate: Group is finalized and locked")
    ...
    current_nominations = db.query(models.Nomination).filter(models.Nomination.group_id == group_id).count()
    if current_nominations >= 3:
        raise HTTPException(status_code=400, detail="Group already has the maximum of 3 nominations")
        
    # Prevent duplicate nominations in the same group
    existing_nom = db.query(models.Nomination).filter(
        models.Nomination.group_id == group_id,
        models.Nomination.restaurant_name == request.restaurant_name
    ).first()
    if existing_nom:
        raise HTTPException(status_code=400, detail="This restaurant has already been nominated in this group")
...
def vote(group_id: int, request: VoteRequest, db: Session = Depends(get_db)):
    ...
    if group.is_locked:
        raise HTTPException(status_code=400, detail="Cannot vote: Group is finalized and locked")
    ...
    if not nomination:
        raise HTTPException(status_code=404, detail="Nomination not found in this group")

    # A user can only vote once per group
    existing_vote = db.query(models.Vote).join(models.Nomination).filter(
        models.Vote.user_id == request.user_id,
        models.Nomination.group_id == group_id
    ).first()
    if existing_vote:
        raise HTTPException(status_code=400, detail="User has already voted in this group")
...
def finalize_group(group_id: int, db: Session = Depends(get_db)):
    ...
    vote_counts = db.query(
        models.Vote.nomination_id,
        func.count(models.Vote.id).label('vote_count')
    ).join(models.Nomination).filter(
        models.Nomination.group_id == group_id
    ).group_by(models.Vote.nomination_id).all()
    
    if not vote_counts:
        # If no votes, either pick a random nomination or leave it None
        nominations = db.query(models.Nomination).filter(models.Nomination.group_id == group_id).all()
        if nominations:
        ...
        tied_nominations = [vc.nomination_id for vc in vote_counts if vc.vote_count == max_votes]
